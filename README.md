...
Your Apple Calendar event notes look like this:

    <p>Weekly Team Sync<br><br>AGENDA:<br>Review project status
    <br>Discuss blockers<br><br>LINK:<br><a href="https://meet.google.com/abc-defg-hij">
    https://meet.google.com/abc-defg-hij</a><br><br>NOTES:<br>
    Bring updated numbers&amp;forecasts</p>
...
Instead of this:
    ...
    NOTES:
    Bring updated numbers & forecasts
...
- Decodes HTML entities (& → &, non-breaking spaces, smart quotes, etc.)
...
Go to [script.google.com](https://script.google.com) and sign in with the Google account that owns (or has edit access to) the calendar.
...
- Copy the entire contents of [Code.gs](Code.gs) and paste it in
...
- Click the **Run** button (the play/triangle icon)
...
**What about events created by third-party apps (Calendly, HubSpot, Zoom, etc.)?**
...
Yes. In the createDailyTrigger function, you can change .everyDays(1) to .everyHours(1) for hourly runs. Be aware of [Apps Script quotas](https://developers.google.com/apps-script/guides/services/quotas) if you have a very full calendar.

**How do I stop it?**
Go to [script.google.com](https://script.google.com), open the project, click the clock icon (Triggers) in the left sidebar, and delete the trigger.

## How It Works (Technical)

The htmlToPlainText function:

1. Extracts URLs from anchor tags and preserves them as raw links
2. Converts block elements (br, p, div, li) to line breaks
3. Converts list items to dash-prefixed lines
4. Strips all remaining HTML tags
5. Decodes HTML entities (amp, nbsp, smart quotes, etc.)
6. Collapses excessive whitespace and blank lines
...
Built by [Sarah Evans](https://github.com/sarahevansai) because every calendar app should just work.
